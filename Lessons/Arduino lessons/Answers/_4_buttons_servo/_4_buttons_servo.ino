...
const int buttonPin1 = 7;        // button connected to pin 7
...
void setup() {                        // start setup, this code runs once at startup
  ...
  pinMode(buttonPin1, INPUT_PULLUP);   // initialize buttonPin as an input and turn on the arduino's internal pull-up resistor on buttonPin
...
void loop() {                         // start main loop  
  if (digitalRead(buttonPin1)){  // this is called a conditional statement. if the true/false condition inside the () is true the code in the {} will execute, if not it will be skipped
    myServo.write(175);               // drive the servo to 175 deg
    delay(50);                        // a .05 second delay to smooth things out, reduces jitter 
  }                                   // end if
  if(digitalRead(buttonPin2)){                              
    myServo.write(90);                // drive the servo to 90 deg
    delay(50);                        // .05 second delay for smoothing
  }                                  
