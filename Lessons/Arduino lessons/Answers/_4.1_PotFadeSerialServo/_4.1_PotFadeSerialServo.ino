...
int motorFadeValue = 0;         // variable used to store the value used to fade the motor 0-180
...
void loop() {                           // start main loop 
  ...
  motorFadeValue = map(sensorValue, 0, 1023, 0, 180); // map the motorFadeValue down from 0-1023 to 0-180
  analogWrite(redLED, fadeValue);       // use the "analogWrite" method to fade the redLED 
  myServo.write(motorFadeValue);        // use the servo.write method to rotate the servo
...
void printValues(){                     // this is a method, it is a block of code that you can call with a single line: printValues(); 
  ...
  Serial.print(fadeValue);              // print the fadeValue (0-255)
