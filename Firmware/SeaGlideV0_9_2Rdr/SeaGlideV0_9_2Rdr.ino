...
static byte rdrCenter = 90;

static int riseDriveTime = 12000;//shallow lead 17000; //lead full19500 ;    //18000; // fornew lead plunger 
static int sampleInterval = 2000;   // sample interval 
...
//static byte encoderPin = 12;         // hall effect sensor on shaft coupling encoder pin
static byte rdrPin = 12;             // rudder servo pin
  ...
  if (checkPause()){
    ...
    while (paused){
      if (irrecv.decode(&results)) {
        ...
        if (results.value == UP){
          Serial.println("BE UP");
          rise(700);          
          flashPurp(50);
        }
        irrecv.resume();
        if (results.value == DOWN){
          Serial.println("BE DOWN");
          dive(700);          
          flashPurp(50);
        }
        if (results.value == LEFT){
          Serial.println("Svo Left");
          rdrPos -= 30;          
          driveRdr();
        }
        irrecv.resume();
        if (results.value == RIGHT){
          Serial.println("Svo Right");
          rdrPos += 30;          
          driveRdr();
        }
        irrecv.resume();
        if (results.value == ENTER){
          Serial.println("Svo Center");
          rdrPos = rdrCenter;          
          driveRdr();
        }
        irrecv.resume();
...
void driveRdr(){
  ...
  myservo.attach(rdrPin);
  myservo.write(rdrPos);
  delay(400);
  myservo.detach();
}
