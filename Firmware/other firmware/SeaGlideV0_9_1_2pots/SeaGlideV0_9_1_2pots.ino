...
//int riseDriveTime = 12500;   //18000; // fornew lead plunger 
static int minRiseDriveT =  8000;    // if the pot is turned all the way to the counter-clockwise the glider will rise-drive for 8 secconds
static int maxRiseDriveT = 21000;    // if the pot is turned all the way to the clockwise the glider will rise-drive for 21 secconds
...
#define TWO 0xFD8877                 // Runs to the end stop and then drives to mid point
...
void loop(){                    // begin main loop
  ...
  rise(readPot2(POT2_PIN));          // Rise: Run the "rise" method. This will start turning the servo to push out water & pitch the glider up
...
int readPot2(int potPin){                       // this method reads a potentiometer to determine the pause time
  int potValue = analogRead(potPin);            // Read the Potentiometer
  int riseDriveTime = map(potValue, 0, 1023, minRiseDriveT, maxRiseDriveT); // scale the value to the diveDriveTime range defined by minDriveTime & maxDriveTime
  Serial.print("Rise Drive Time: ");            // print a lable to the serial port
  Serial.println(riseDriveTime);                // print the pause time value to the serial port
  return riseDriveTime;                         // return the pause time, an intiger (int) value
}                                               // end of method
