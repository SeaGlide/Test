...
// Pins 
static byte SERVO_PIN = 7;          // the pin that the "continuous rotation servo" is attached to, this motor drives the buoyancy engine
static byte DIVE_STOP = 8;          // the pin that the dive limmit switch (round push button) is attached to
...
static byte POT_PIN = A0;            // the pin that the wiper of the little orange trim pot is attached to
static byte RECV_PIN = 12;            // IR reciever signal pin
//static byte IR_GND = 3;              // IR middle pin, ground
//static byte IR_PWR = 4;              // IR power pin
static byte encoderPin = 3

static byte RED_LED = 4;             // these are the three pins that the RED
static byte GREEN_LED = 5;           //                                   GREEN
static byte BLUE_LED = 6;            //                               and BLUE LED cathodes are attached to
...
void loop(){                   // begin main loop
  ...
  rise(150); //150                     // Rise: Run the "rise" method. This will start turning the servo to push out water & pitch the glider up
